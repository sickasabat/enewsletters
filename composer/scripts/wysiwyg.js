...
var WYSIWYG = {
	...
	Settings: function() {
		...
		this.DisabledStyle = "font-family: Arial; font-size: 12px; background-color: #FFFFFF";
	...
	"backcolor":      ['BackColor',            'Highlight',          	'highlight.gif',          'highlight_on.gif'],
	//"background":	  ['Background',	   'Back Color',		'backcolor.gif',	  'backcolor_on.gif'],
	...
	insertImage: function(src, width, height, align, border, alt, hspace, vspace, n) {
		...
		//if(link != "") { WYSIWYG_Core.setAttribute(img,"href",link); } else { img.removeAttribute("href");}
		if(align != "") { WYSIWYG_Core.setAttribute(img, "align", align); } else { img.removeAttribute("align"); }
		if(border != "") {WYSIWYG_Core.setAttribute(img, "border", border);} else {img.removeAttribute("border");}
		if(alt != "") {WYSIWYG_Core.setAttribute(img, "alt", alt);} else{img.removeAttribute("alt");}
		if(hspace != ""){WYSIWYG_Core.setAttribute(img, "hspace", hspace);}else{img.removeAttribute("hspace");}
		if(vspace != ""){WYSIWYG_Core.setAttribute(img, "vspace", vspace);}else{img.removeAttribute("vspace");}
	...
	_display: function(n, settings) {
		...
		var iframe = '<table id="WYSIWYGtable" cellpadding="0" cellspacing="0" border="0" style="width:' + currentWidth + '; height:' + currentHeight + ';" class="tableTextareaEditor"><tr><td valign="top">\n'
	...
	execCommand: function(n, cmd, value) {
		...
		switch(cmd) {
			...
			//Background Color
			/*case "Background":
				var currentColor = this.getEditorWindow(n).document.body.bgcolor;//queryCommandValue(cmd));
				alert(currentColor);
				window.open(this.config[n].PopupsDir+ 'select_color.html?color=' + currentColor + '&command=' + cmd +'&wysiwyg=' + n, 'popup', 'location=0,status=0,scrollbars=0,width=210,height=165,top=' + popupPosition.top + ',left=' + popupPosition.left).focus();
			break;
			*/
...
var WYSIWYG_Core = {
	...
	execCommand: function(n, cmd, value) {
		...
		/*
		if(cmd == 'Background') {
			WYSIWYG.getEditorWindow(n).document.body.bgcolor = value;
			alert(WYSIWYG.getName());
			jQuery(WYSIWYG.getName()).css({'background-color':value});
			jQuery('iframe').css({'background-color':value});
			jQuery('#text2').css({'background-color': value});
		}
		
		// firefox cut, paste and copy
		else*/ if(WYSIWYG_Core.isFF && (cmd == "Cut" || cmd == "Paste" || cmd == "Copy")) {
			try {
